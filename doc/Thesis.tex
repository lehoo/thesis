...
I, Attila Döme Lehóczky, declare that this thesis titled, `Validation
and improvement of the SMPI simulation framework for MPI applications' and the work presented in it are my own. I confirm that:
...
easily switch between configurations, producing experiment results on
more than one such system without a large overhead. SimGrid is a
...
protocol, one of the most prominently used in distributed computing
currently. SMPI is a framework that connects SimGrid and MPI: it's
part of the SimGrid project with the goal of simulating MPI
applications in a simulated environment. SMPI is currently under heavy
development. Creating generic network models is an ongoing effort and
it is one of the development directions that gets the most focus by
the development team. This process needs a lot of validation, which is
...
different steps. It's tedious, repetitive work, but as mentioned
before, it's necessary. This thesis is about the
...
\input{./Chapters/Chapter3} % Problem Description

\input{./Chapters/Chapter4} % Implementation

\input{./Chapters/Chapter5} % Evaluation Plan
